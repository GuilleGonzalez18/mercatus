...
try {
  ...
  // Now create the cfeBuilder.js file
...
`;
  ...
  fs.writeFileSync(filePath, cfeBuilderContent, 'utf8');
  console.log('✓ cfeBuilder.js file created successfully:', filePath);
...
`;
  ...
  const cfeBuilderScript = path.join('D:\\repos\\ferco-posta', 'setup-cfe-builder.js');
  fs.writeFileSync(cfeBuilderScript, setupScript, 'utf8');
  console.log('✓ Setup script created at:', cfeBuilderScript);
  console.log('\nNow run: node setup-cfe-builder.js');
} catch (err) {
  console.error('Error creating setup script:', err.message);
}
